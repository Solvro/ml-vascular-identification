...
def compute_eer(
    ...
    # Find where FAR == FRR
    diff = np.absolute(fpr - frr)

    # Handle case where all differences are NaN (e.g., no positive/negative samples)
    if np.all(np.isnan(diff)):
        return 50.0, 0.5  # Return default values when metric cannot be computed
...
def compute_tpr_at_fpr(
    ...
    """
    # Handle edge case: insufficient samples
    ...
    diff = np.absolute(fpr - target_fpr)
    if np.all(np.isnan(diff)):
        return float("nan"), float("nan")
...
def compute_oscr(
    ...
    if thresholds is None:
        ...
        thresholds = np.linspace(all_scores.min() - 0.1, all_scores.max() + 0.1, 200)
    ...
    for threshold in thresholds:
        # CCR: fraction of known samples correctly classified AND above threshold
        correct_and_accepted = (known_predictions == known_labels) & (
...
def compute_openset_metrics(
    ...
    if len(unknown_scores) == 0:
        ...
        tpr_001, tpr_threshold_001 = compute_tpr_at_fpr(
            known_scores, unknown_scores, target_fpr=0.001
        )
        metrics["tpr_at_fpr_0.001"] = tpr_001
        metrics["tpr_threshold_0.001"] = tpr_threshold_001

        # TPR at FPR=0.01 (1%)
        tpr_01, tpr_threshold_01 = compute_tpr_at_fpr(
            known_scores, unknown_scores, target_fpr=0.01
        )
        metrics["tpr_at_fpr_0.01"] = tpr_01
        metrics["tpr_threshold_0.01"] = tpr_threshold_01

        # TPR at FPR=0.1 (10%)
        tpr_1, tpr_threshold_1 = compute_tpr_at_fpr(
            known_scores, unknown_scores, target_fpr=0.1
        )
        ...
        try:
            oscr_result = compute_oscr(
                known_predictions, known_labels, known_scores, unknown_scores
            )
