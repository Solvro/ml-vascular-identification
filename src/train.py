...
def find_optimal_threshold(
    model, val_known_loader, val_unknown_loader, prototypes, device,
    class_to_idx=None, metric='oscr'
):
    """
    ...
    thresholds = np.linspace(
        all_similarities.min() - 0.1,
        all_similarities.max() + 0.1,
        200
    )
...
def evaluate_openset(
    ...
    class_to_idx=None, threshold=0.5, k=1
):
    """
    ...
    correct_known = sum(
        1 for sim, pred, true_cls in zip(
            all_similarities[:len(all_true_classes)], 
            all_predictions, 
            all_true_classes
        )
    ...
    correct_unknown = sum(
        1 for sim in all_similarities[len(all_true_classes):]
        if sim < threshold
    )
...
def train(cfg: DictConfig) -> None:
    ...
    if mode == "openset":
        ...
        test_known_enrollment_loader = loaders.get('test_known_enrollment', loaders.get('test_known', None))
        test_known_query_loader = loaders.get('test_known_query', loaders.get('test_known', None))
    ...
    print(f"📉 Loss: {loss_name} ({loss_info_str if loss_info_str else 'default params'})")
    ...
    if mode == "openset" and test_unknown_loader is not None:
        ...
        from utils import plot_det_curve, plot_roc_curve, save_evaluation_protocol, InferenceTimer
        ...
        enrollment_loader = loaders.get('test_known_enrollment', val_loader)  # Fallback to val if not available
        query_known_loader = loaders.get('test_known_query', loaders.get('test_known', val_loader))
        ...
        print(f"✅ Computed {len(prototypes)} prototypes from {len(enrollment_loader.dataset)} enrollment samples")
        ...
        if optimize_threshold:
            ...
            optimal_threshold, best_metric_val = find_optimal_threshold(
                model, val_loader, test_unknown_loader, prototypes, device,
                class_to_idx, metric=threshold_metric
            )
            threshold = optimal_threshold
            print(f"✅ Optimal threshold: {threshold:.4f} ({threshold_metric}={best_metric_val:.4f})")
        else:
        ...
        with InferenceTimer("OpenSet evaluation (query)") as timer:
            metrics = evaluate_openset(
                model, query_known_loader, test_unknown_loader, prototypes, device, 
                class_to_idx, threshold, k=k_neighbors
            )
        ...
        metrics['avg_query_time_per_sample_ms'] = query_time_ms / (
            len(query_known_loader.dataset) + len(test_unknown_loader.dataset)
        ) if hasattr(query_known_loader, 'dataset') else 0.0
        ...
        print(f"   Query (per sample): {metrics['avg_query_time_per_sample_ms']:.2f} ms")
        ...
        protocol_config = {
            ...
            'test_known_size': len(query_known_loader.dataset) if hasattr(query_known_loader, 'dataset') else 0,
            'test_unknown_size': len(test_unknown_loader.dataset) if hasattr(test_unknown_loader, 'dataset') else 0,
            'enrollment_size': len(enrollment_loader.dataset) if hasattr(enrollment_loader, 'dataset') else 0,
            'model_name': model_name,
            'embedding_dim': embedding_dim,
            ...
            'k': k_neighbors,
            'threshold': threshold,
            'threshold_optimization': threshold_metric if optimize_threshold else 'manual',
        }
